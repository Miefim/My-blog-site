...
import saleCards from '../../PseudoAPI/sales.json'
import Slider from '../../UI/Slider'
import MiniSlider from '../../UI/MiniSlider'
import MaterialDropDown from '../../UI/DropDown'
import style from './index.module.css'

const Sales = () => {
   const categoryArray = ['Кольца', 'Серьги', 'Браслеты', 'Часы']
   const [activCard, setActiveCard] = useState(null)
   ...
   const filterCards = useMemo(() => {
      return saleCards.filter((el) => {
         return el['type'] === category
      })
   },[category])

   const calculateSale = (price, oldPrice) => {
      price = Number(price.replace(/ /g,''))
      oldPrice = Number(oldPrice.replace(/ /g,''))
      return `-${Math.round(100 - (price / (0.01 * oldPrice)))}%`
   }

   return (
      <div className={style.sales} onMouseLeave={() => {setActiveCard(null)}}>
         <h1 className={style.sales_title}>Распродажа</h1>
         <div className={style.sales_categoryLine}>
            {categoryArray.map((el, i) => 
               <div  
                  className={i === category ? [style.categoryLine_btn, style.categoryLine_btn_active].join(' ') : style.categoryLine_btn} 
                  key={i}
         ...
         <Slider 
            sliderClass={activCard !== null ? [style.slider, style.sliderActiveCard].join(' ') : style.slider}
            btnRClass={style.buttonRightSlider}
            btnLClass={style.buttonLeftSlider}
            circleGroupClass={style.sliderCircleGroup}
         >
            { 
               filterCards.map((card, i) => 
                  <div 
                     className={activCard === i ? [style.card, style.cardActive].join(' ') : style.card} 
                     onMouseOver={() => setActiveCard(i)}
                     key={card.id}
                  >
                     {card.oldPrice && <div className={[style.saleIcon, style.saleIcon_green, style.saleIcon_visible].join(' ')}>Sale</div>}
            ...
            {/* <div id='2' className={activCard === '2' ? [style.card, style.cardActive].join(' ') : style.card} onMouseOver={selectActiveCard}>
            ...
            <div id='3' className={activCard === '3' ? [style.card, style.cardActive].join(' ') : style.card} onMouseOver={selectActiveCard}>
               <div className={[style.saleIcon, style.saleIcon_green].join(' ')}>Sale</div>
               <div className={style.likeIconBlock}>
                  <svg width="20" height="18" viewBox="0 0 20 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                     <path className={style.likeIcon} d="M7.94668e-08 5.40001C-0.000248434 3.95035 0.582385 2.56146 1.61681 1.54584C2.65124 0.53022 4.05058 -0.026834 5.5 1.1704e-05C7.21732 -0.0091083 8.85599 0.719186 10 2.00001C11.144 0.719186 12.7827 -0.0091083 14.5 1.1704e-05C15.9494 -0.026834 17.3488 0.53022 18.3832 1.54584C19.4176 2.56146 20.0002 3.95035 20 5.40001C20 10.756 13.621 14.8 10 18C6.387 14.773 7.94668e-08 10.76 7.94668e-08 5.40001Z" fill="#000000"/>
            ...
            <div id='4' className={activCard === '4' ? [style.card, style.cardActive].join(' ') : style.card} onMouseOver={selectActiveCard}>
               <div className={[style.saleIcon, style.saleIcon_green].join(' ')}>Sale</div>
               <div className={style.likeIconBlock}>
                  <svg width="20" height="18" viewBox="0 0 20 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                     <path className={style.likeIcon} d="M7.94668e-08 5.40001C-0.000248434 3.95035 0.582385 2.56146 1.61681 1.54584C2.65124 0.53022 4.05058 -0.026834 5.5 1.1704e-05C7.21732 -0.0091083 8.85599 0.719186 10 2.00001C11.144 0.719186 12.7827 -0.0091083 14.5 1.1704e-05C15.9494 -0.026834 17.3488 0.53022 18.3832 1.54584C19.4176 2.56146 20.0002 3.95035 20 5.40001C20 10.756 13.621 14.8 10 18C6.387 14.773 7.94668e-08 10.76 7.94668e-08 5.40001Z" fill="#000000"/>
            ...
            <div id='5' className={activCard === '5' ? [style.card, style.cardActive].join(' ') : style.card} onMouseOver={selectActiveCard}>
               <div className={[style.saleIcon, style.saleIcon_green].join(' ')}>Sale</div>
               <div className={style.likeIconBlock}>
                  <svg width="20" height="18" viewBox="0 0 20 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                     <path className={style.likeIcon} d="M7.94668e-08 5.40001C-0.000248434 3.95035 0.582385 2.56146 1.61681 1.54584C2.65124 0.53022 4.05058 -0.026834 5.5 1.1704e-05C7.21732 -0.0091083 8.85599 0.719186 10 2.00001C11.144 0.719186 12.7827 -0.0091083 14.5 1.1704e-05C15.9494 -0.026834 17.3488 0.53022 18.3832 1.54584C19.4176 2.56146 20.0002 3.95035 20 5.40001C20 10.756 13.621 14.8 10 18C6.387 14.773 7.94668e-08 10.76 7.94668e-08 5.40001Z" fill="#000000"/>
